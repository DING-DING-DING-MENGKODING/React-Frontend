import { Calendar, Droplets, Minus, Plus, User, X, Loader2, ChevronsUpDown } from "lucide-react";
import React, { useState, useEffect } from "react";

export default function KantongDarah() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [stockRes, logRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/log?tipe_item=darah`, { headers }),
      ]);
      ...
      setStockData(stockJson.stok_darah || []);
  ...
  const ModalForm = ({ onCancel, onSubmit }) => (
    <div className="space-y-4">
        <div>
          ...
          >
            <option value="" disabled>Pilih jenis darah</option>
            {stockData.map(item => (
              <option key={item.jenis_darah} value={item.jenis_darah}>{item.jenis_darah}</option>
            ))}
          </select>
        </div>
        <div>
        ...
        <div className="flex space-x-3 mt-6">
          <button onClick={onSubmit} className="flex-1 bg-[#E30030] text-[#FFFFFF] py-2 rounded-lg hover:bg-opacity-90 transition-colors">
            Simpan
          </button>
          <button onClick={onCancel} className="flex-1 bg-[#80808A] text-[#FFFFFF] py-2 rounded-lg hover:bg-opacity-90 transition-colors">
            Batal
          </button>
        </div>
    </div>
    ...
    <>
      <div className="space-y-10">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200/50">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">Rincian Stok Darah</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 text-center">
            {stockData.map(item => (
              <div key={item.jenis_darah} className="bg-gray-100 p-4 rounded-lg">
                <p className="text-sm font-bold text-red-600">{item.jenis_darah}</p>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="bg-[#E30030]/10 px-6 py-2 rounded-lg">
              <span className="text-[#E30030] font-semibold">
                Total Stok: {totalStock} Kantong
              </span>
            </div>
          </div>
          <div className="divide-y divide-[#80808A]/10">
            {logData.map((log) => (
              <div key={log.id} className="flex items-start justify-between py-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="font-semibold text-[#1F1F1F]">
                      {log.type === "in"
                        ? `Penambahan Stok (${log.jenis})`
                        : `Pengurangan Stok (${log.jenis})`}
                    </h3>
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-[#1F1F1F]/50 flex items-center justify-center z-50 p-4">
            <div className="bg-[#FFFFFF] rounded-xl p-6 w-full max-w-md">
              ...
              <ModalForm onCancel={() => setShowAddModal(false)} onSubmit={() => handleTransaction('in')} />
            </div>
          </div>
        )}
        ...
        {showRemoveModal && (
          <div className="fixed inset-0 bg-[#1F1F1F]/50 flex items-center justify-center z-50 p-4">
            <div className="bg-[#FFFFFF] rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-[#1F1F1F]">Form Pengambilan Kantong Darah</h3>
                <button
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[#80808A] mb-2">
                    Jenis Darah <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={selectedBloodType}
                    onChange={(e) => setSelectedBloodType(e.target.value)}
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Informasi:</strong> Setelah form disubmit, stok akan otomatis berkurang sebanyak <strong>1 kantong darah</strong>.
                  </p>
                </div>
